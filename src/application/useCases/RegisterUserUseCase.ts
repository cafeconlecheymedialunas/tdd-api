...
export class RegisterUserUseCase implements RegisterUseCaseInterface {
  ...
  async register({ name, email, password, roles }: { name: string, email: string, password: string, roles: number[] }): Promise<UserDto> {
    if (email === '' || password === '' || name === '') throw new ClientError('Email, name and password are required fields', HttpStatuses.FORBIDDEN)
    const user = await this.repository.getUserByEmail(email);
    if (user !== undefined) throw new ClientError('There is already a user with that email', HttpStatuses.FORBIDDEN)
    const passwordHash = await this.hash.hash(password)
    if (!passwordHash) throw new ClientError('The username or password does not match', HttpStatuses.UNAUTHORIZED)
